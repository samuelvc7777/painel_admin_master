...
import {
  ...
  type BillingPlanBreakdown as BillingPlanBreakdownItem,
  type BillingSectionState,
} from "@/lib/billing";

import { BillingEmptyState } from "./billing-empty-state";

interface BillingPlanBreakdownProps {
  items: BillingPlanBreakdownItem[];
  state?: BillingSectionState;
}

export function BillingPlanBreakdown({ items, state }: BillingPlanBreakdownProps) {
  const highestRevenue = Math.max(...items.map((item) => item.estimatedMrrCents), 1);
  ...
  return (
    <section className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
            Participacao no MRR, novos clientes e cancelamentos do periodo.
          </p>
        </div>
        <Link
          href="/plans"
      ...
      <div className="mt-5 space-y-3">
        {items.length === 0 ? (
          <BillingEmptyState
            title={state?.title ?? "Sem planos no recorte"}
            message={state?.message ?? "A receita por plano aparece quando houver planos ou assinaturas relacionadas."}
            ...
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    {item.paidSubscriberCount} pagos - {item.trialSubscriberCount} trials - {formatBillingPercent(item.sharePercent)} do MRR
                  </p>
                </div>
                <div className="grid grid-cols-3 gap-3 text-left lg:min-w-[20rem]">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">MRR</p>
                    <p className="mt-1 text-sm font-black text-slate-950 dark:text-white">
                      {formatCurrencyFromCentsForBilling(item.estimatedMrrCents)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Novas</p>
                    <p className="mt-1 text-sm font-black text-emerald-600 dark:text-emerald-300">
                      {item.newSubscriptionsCount}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Cancel.</p>
                    <p className="mt-1 text-sm font-black text-red-600 dark:text-red-300">
              ...
              <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-800">
                <div
                  ...
                  style={{ width: `${item.estimatedMrrCents > 0 ? Math.max(8, (item.estimatedMrrCents / highestRevenue) * 100) : 0}%` }}
                />
              </div>
            </Link>
          ))
